  ...
  const saved = localStorage.getItem("motor1");
  return saved ? JSON.parse(saved) : false;
  });

  const [m2,setM2]=useState(()=>{
  const saved = localStorage.getItem("motor2");
  return saved ? JSON.parse(saved) : false;
  });
  ...
  localStorage.setItem(motor.label,JSON.stringify(newState));
...
ws = new WebSocket("wss://green-house-backend.onrender.com");
...
if(msg.topic === "greenhouse/air/temperature")
updateChart("temp", value);

if(msg.topic === "greenhouse/air/humidity")
updateChart("hum", value);

if(msg.topic === "greenhouse/air/co2")
updateChart("co2", value);
